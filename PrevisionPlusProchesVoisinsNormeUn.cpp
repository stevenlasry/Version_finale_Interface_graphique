...
// 1. Implementation of a function which returns the One Norm

double PrevisionPlusProchesVoisinsNormeUn::Normeun(QVector<double> const& a, QVector<double> const& b )
{
    double resultat(0);
    int taille(a.size());
    ...
    for(int i(0) ; i < taille ; ++i)
    {resultat += Valeurabsolue(difference[i]);
    }
    return resultat;
}
